...
echo "Updating GoCommit..."

# Check if GoCommit is currently installed
if ! command -v gocommit &> /dev/null; then
    echo "Warning: GoCommit is not currently installed. Use install.sh to install it first."
    ...
    if [[ "$response" =~ ^([yY][eE][sS]|[yY])$ ]]; then
        if [ -f "install.sh" ]; then
            ...
            echo "Error: install.sh not found. Please download the repository first."
            exit 1
        fi
    else
...
echo "Current GoCommit installation found at: $(which gocommit)"
...
echo "Downloading latest GoCommit..."
...
if [ -f "$INSTALL_DIR/gocommit" ]; then
    echo "Backing up current version..."
    sudo cp "$INSTALL_DIR/gocommit" "$INSTALL_DIR/gocommit.backup.$(date +%Y%m%d_%H%M%S)"
fi

# Install the updated binary
echo "Installing updated version to $INSTALL_DIR..."
...
echo "Update complete! GoCommit has been successfully updated."
echo "Your API key and configuration are preserved."
echo ""
echo "You can now use the updated 'gocommit' from anywhere."
echo "Run 'gocommit --help' to see if there are any new features."
